...
- **deps**: 開発依存の脆弱性対応 — undici 7.29.0 / postcss 8.5.25 へ更新 (#504)



### ドキュメント

- /pre-release にリリース後の告知手順（GitHub Release + labs リリースノート）を追加 (#505)



### バグ修正

- ナビのタブを往復しても絞り込み・比較対象を保持する (#482)



### 新機能

- データ管理画面（/data）— 保有ヒストリカルデータの一覧と鮮度表示 (#494)


- データ画面から GUI でヒストリカルデータを取得・一括更新できるようにする (#495)


- **front**: no_data と開発画面からデータ取得へのプリフィル付き導線を追加 (#496)


- 詳細画面から TradingView 用 Pine Script を生成・プレビュー・ダウンロードできるようにする (#497)


- Pine 生成前の非対応指標警告・Trial アップグレード導線・貼り付けガイドを追加 (#498)


- **front**: 開発画面にゴールビルダー（選択式でゴール文を組み立て）を追加 (#499)


- **front**: AI 戦略開発の完了パネルに次アクション導線を追加 (#500)


- 既存戦略を起点にした AI 派生開発（改善指示で再実行）を追加 (#501)


- セットアップ状態 API と「はじめる」画面（チェックリスト）を追加 (#502)


- 「はじめての戦略作成」ステップガイドと未セットアップ時のナビ強調を追加 (#503)
...
- ターン上限到達を正しく伝え、EULA の誤診断をやめる (#480)
