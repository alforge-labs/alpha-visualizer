...
- **Command-execution endpoints exist.** `POST /api/run`, `POST /api/jobs`, and `POST /api/strategies/{id}/parameters` execute the `forge` CLI found on PATH as a subprocess (arguments are passed as a list without a shell; strategy_id / symbol are passed as positionals after an `--` end-of-options marker). Note that `POST /api/strategies/{id}/parameters` **modifies strategy definitions**. If bound to a non-localhost address, anyone who can reach the server can start backtests / optimizations and change strategy parameters, so **exposing the server publicly is not recommended**. Home directory paths in responses and logs are masked as `~`.
