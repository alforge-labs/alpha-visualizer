...
- **Command-execution endpoints exist.** `POST /api/run` and `POST /api/jobs` execute the `forge` CLI found on PATH as a subprocess (arguments are passed as a list without a shell; strategy_id / symbol are passed as positionals after an `--` end-of-options marker). If bound to a non-localhost address, anyone who can reach the server can start backtests and optimizations, so **exposing the server publicly is not recommended**. Home directory paths in responses and logs are masked as `~`.
