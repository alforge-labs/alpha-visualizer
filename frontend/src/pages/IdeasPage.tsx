...
function statusLabel(status: Status, lang: 'ja' | 'en'): string {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      >
        <div style={{ display: 'flex', gap: 'var(--space-2)', flexWrap: 'wrap' }}>
          <StatusButton
            label={L('すべて', 'All')}
            active={list.statusFilter === ''}
            onClick={() => list.setStatusFilter('')}
          />
          {STATUS_VALUES.map(s => (
            <StatusButton
              key={s}
              label={statusLabel(s, lang)}
              active={list.statusFilter === s}
              onClick={() => list.setStatusFilter(list.statusFilter === s ? '' : s)}
            />
          ))}
        </div>
          ...
          >
            {list.allTags.map(tag => (
              <button
                key={tag}
                type="button"
                aria-pressed={list.tagFilter === tag}
                onClick={() => list.setTagFilter(list.tagFilter === tag ? '' : tag)}
                style={{
                  padding: '3px 10px',
                  background: list.tagFilter === tag ? 'var(--accent)' : 'var(--surface-2)',
                  border: `1px solid ${list.tagFilter === tag ? 'var(--accent)' : 'var(--border)'}`,
                  borderRadius: 'var(--radius-pill)',
                  fontFamily: 'var(--mono)',
                  fontSize: 'var(--fs-mono-sm)',
                  fontWeight: 600,
                  color: list.tagFilter === tag ? 'var(--surface)' : 'var(--text2)',
                  letterSpacing: 'var(--tracking-mono)',
                  cursor: 'pointer',
                  textTransform: 'uppercase',
      ...
      <main style={{ flex: 1, padding: 'var(--space-6) var(--space-7)' }}>
        {list.loading ? (
          ...
          <EmptyState lang={lang} hasFilter={list.statusFilter !== '' || list.tagFilter !== ''} />
        ) : (
          <div
            style={{
              display: 'grid',
