...
describe('LiveKpiRow', () => {
  ...
  it('ベンチマークがあれば超過リターンを表示する', () => {
    // WHY: benchmarkEquity は呼び出し側（LivePositionView）が equity と
    // 同じ長さの系列だけを許容してから渡す契約になった（Finding 1 対応）。
    // ここでも equity（3 点）と同じ長さの系列を渡し、その契約を反映する。
    render(
      <LiveKpiRow
        equity={EQUITY}
        benchmarkEquity={[
  ...
  it('ベンチマークとBTが両方あれば、それぞれ別の値を表示する', () => {
    ...
    // equity（3 点）と同じ長さの系列を渡す（Finding 1: 呼び出し側の契約）。
    render(
      <LiveKpiRow
        equity={EQUITY}
        benchmarkEquity={[
